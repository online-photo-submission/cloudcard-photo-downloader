...
class CCureStorageService implements StorageService {

    static final Logger log = LoggerFactory.getLogger(CCureStorageService.class);
    ...
    List<PhotoFile> save(Collection<Photo> photos) {
        ...
        log.info("Uploading Photos to CCure")

        String sessionId = cCureClient.operatorLogin()

        if (!sessionId) {
            log.error("Failed to login to the CCure API")
            return []
        }
    ...
    PhotoFile save(Photo photo, String sessionId) {
        if (!photo.person) {
            log.error("Person does not exist for photo $photo.id and it cannot be uploaded to CCure.")
            return null
        }

        log.info("Uploading to CCure: $photo.id for person: $photo.person.email")
        ...
        if (!cCureClient.accountPhotoApprove(sessionId, accountId, photoBase64)) {
            log.error("Photo $photo.id for $photo.person.email failed to upload into CCure.")
            return null
        }
    ...
    String resolveAccountId(Photo photo) {
        ...
        if (!accountId) {
            log.error("We could not resolve the accountId for '$photo.person.email' with ID number '$photo.person.identifier', so photo $photo.id cannot be uploaded to CCure.")
            return null
        }
    ...
    String getBytesBase64(Photo photo) {
        if (!photo.bytes) {
            log.error("Photo $photo.id for $photo.person.email is missing binary data, so it cannot be uploaded to CCure.")
            return null
        }
