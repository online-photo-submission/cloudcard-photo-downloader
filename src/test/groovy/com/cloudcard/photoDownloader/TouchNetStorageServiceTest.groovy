package com.cloudcard.photoDownloader;

import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.api.extension.ExtendWith;
import org.mockito.InjectMocks;
import org.mockito.Mock;
import org.mockito.junit.jupiter.MockitoExtension;
import org.springframework.test.util.ReflectionTestUtils;

import java.util.ArrayList;
import java.util.List;
import java.util.Base64;

import static org.assertj.core.api.Assertions.assertThat;
import static org.mockito.Mockito.*;
@ExtendWith(MockitoExtension.class)
public class TouchNetStorageServiceTest {

    @Mock
    TouchNetClient touchNetClient

    TouchNetStorageService service

    String base64Photo = "";

    @BeforeEach
    public void setUp() {
        service = new TouchNetStorageService()
        service.touchNetClient = touchNetClient
    }

    //todo spockify these jawnies

    @Test
    public void testSaveWithNoPhotos() throws Exception {
        //setup
        List<Photo> photos = [];

        //test
        List<PhotoFile> photoFiles = service.save(photos);

        //verify
        assert photoFiles.size() == 0;

        verify(touchNetClient, times(1)).operatorLogin();
        verify(touchNetClient, times(1)).operatorLogout(//todo sessionID);
    }

    @Test
    public void testSaveWithOneGoodPhoto() throws Exception {
        List<Photo> photos = new ArrayList<Photo>();

        Photo photo = new Photo();
        Person person = new Person();
        person.setIdentifier("100044616");
//        person.setIdentifier("100044617");
        photo.setPerson(person);
        photo.setBytes(Base64.getDecoder().decode(base64Photo));
        photos.add(photo);

        List<PhotoFile> photoFiles = service.save(photos);

        assert photoFiles.size() == 1;
        assert photoFiles.get(0).getBaseName() == "100044616";
    }


    @Test
    public void testSaveWithThreeGoodPhotos() throws Exception {

    }

    @Test
    public void testSaveWithOneBadAndTwoGoodPhotos() throws Exception {

    }

    @Test
    public void testSaveWhenLoginFails() throws Exception {

    }

    @Test
    public void testSaveWhenOneUploadFails() throws Exception {

    }

    @Test
    public void testSaveWhenLogoutFails() throws Exception {

    }

    @Test
    public void testSaveWithPhotoMissingBytes() throws Exception {

    }

    @Test
    public void testSaveWithPhotoMissingPerson() throws Exception {

    }

    @Test
    public void testSaveWithPhotoMissingAccountId() throws Exception {

    }

}
