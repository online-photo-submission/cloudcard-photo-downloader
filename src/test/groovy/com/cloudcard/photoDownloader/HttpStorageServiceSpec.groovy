...
class HttpStorageServiceSpec extends Specification{

    HttpStorageService service = new HttpStorageService()
    ...
    void setup() {
        service.httpStorageClient = Mock(HttpStorageClient)
    ...
    void "test save with no photos"() {
        ...
        and: "no unexpected mock invocations"
        0 * _
    }
    ...
    void "test save with one good photo"() {
        ...
        _ * service.httpStorageClient.getSystemName() >> "HTTP Storage Client"
        1 * service.fileNameResolver.getBaseName(photo) >> identifier
        1 * service.httpStorageClient.putPhoto(identifier, base64Photo)
        1 * service.httpStorageClient.close()
    ...
    void "test save with three good photos"() {
        ...
        1 * service.httpStorageClient.putPhoto(identifiers[0], Base64.encoder.encodeToString([0, 1, 2, 3, 0] as byte[]))
        1 * service.httpStorageClient.putPhoto(identifiers[1], Base64.encoder.encodeToString([0, 1, 2, 3, 1] as byte[]))
        1 * service.httpStorageClient.putPhoto(identifiers[2], Base64.encoder.encodeToString([0, 1, 2, 3, 2] as byte[]))
        1 * service.httpStorageClient.close()
        0 * _
    }
    ...
    void "test save when uploadPhoto fails"() {
        given:
        String identifier = "100012345"
        Photo photo = new Photo(
                id: 1,
    ...
    void "test save when accountId not resolvable"() {
        ...
        0 * service.httpStorageClient.putPhoto(_,_)
        0 * _
    }
